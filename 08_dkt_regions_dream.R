...
dkt <- read.csv(paste0(scratch_path, "/processed_data/DKT_stats_combat_averaged.csv")) %>% rename(projid = Subject)

data <- combined_data %>%
  filter(gsub(" ", "", tissue) == tissue) %>%
  left_join(dkt[, c('projid', region_feature)], by = 'projid') %>%
  column_to_rownames('specimenID')

rna_this <- rnaseq_subset_filtered[, rownames(data)]  # the fix from before, applied once, everywhere

formula <- as.formula(paste0("~ `", region_feature, "` + age_diff + age_death + master_brain_side + msex"))
fit <- dream(rna_this, formula, data)
fit <- eBayes(fit)

tt <- topTable(fit, coef = paste0("`", region_feature, "`"), number = Inf)
out_dir <- paste0(scratch_path, "/processed_data/toptables/dkt/", tissue, "/")
